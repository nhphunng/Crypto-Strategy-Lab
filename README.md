...
| Strategy và Backtest/Evaluation backend | `backend/src/crypto_lab/` | Đã có Strategy Foundation, backtest deterministic, metrics, scoring, comparison, REST API, PostgreSQL schema và repository; còn các gate integration/reliability cần PostgreSQL test |
...
| `004-backtest-evaluation` | 54/64 | Đã có deterministic engine, accounting, metrics, scoring, comparison, persistence, REST API và test; còn PostgreSQL/API reliability gate |
| `005-leaderboard-visualization` | 52/52 | Hoàn thành Top-K projection, live update, visualization, test, k6 và E2E |

Tổng theo năm feature chính: **309/323 task, khoảng 96%**. Tỷ lệ này chỉ thể hiện số checkbox, không phải phần trăm effort vì độ lớn mỗi task khác nhau.
