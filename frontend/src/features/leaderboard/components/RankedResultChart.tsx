/**
 * Ranked-result candlestick surface.
 *
 * A dependency-free SVG chart that owns scales and candle rendering only and
 * extends through the `overlays` and `markers` render inputs. It is separate
 * from the realtime market-chart component (feature 002), which is built on
 * lightweight-charts and exposes no scale for shape-based markers.
 *
 * Promotion to `shared/charts` waits for reuse by a second feature.
 */
...
export type RankedResultChartProps = {
  candles: ChartCandle[]
  height?: number
  emptyMessage?: string
...
export function RankedResultChart({
  candles,
  height = 320,
