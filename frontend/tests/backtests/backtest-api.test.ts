...
import type { CandleDataset, DatasetCandle } from '../../src/features/backtests'
...
const completeDataset: CandleDataset = {
  ...
  status: 'COMPLETE', candleCount: 2, checksum: 'a'.repeat(64), failureCode: null,
  createdAt: '2026-08-24T00:00:00.000Z', updatedAt: '2026-08-24T00:00:00.000Z', completedAt: '2026-08-24T00:00:00.000Z',
}

const datasetCandles: DatasetCandle[] = [
  { provider: 'BINANCE', pair: 'BTCUSDT', timeframe: '15m', openTime: '2026-08-01T00:00:00.000Z', closeTime: '2026-08-01T00:14:59.999Z', open: '99', high: '101', low: '98', close: '100', volume: '10', closed: true, receivedAt: '2026-08-01T00:15:00.000Z' },
  { provider: 'BINANCE', pair: 'BTCUSDT', timeframe: '15m', openTime: '2026-08-01T00:15:00.000Z', closeTime: '2026-08-01T00:29:59.999Z', open: '100', high: '102', low: '99', close: '101', volume: '10', closed: true, receivedAt: '2026-08-01T00:30:00.000Z' },
]
...
describe('Feature 004 REST workflow', () => {
  it('sends exact immutable identities and returns backend-owned results', async () => {
    ...
    const fetcher = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.pathname === '/api/v1/market-data/datasets' && init?.method === 'POST') return page(completeDataset, 201)
      ...
      if (url.pathname.endsWith('/start')) return page({
        ...
        initialCapital: '10000', finalEquity: '10100', signalCount: 2, tradeCount: 1, equityPointCount: 1,
      ...
      if (url.pathname.endsWith('/candles')) {
        candlePage += 1
        const firstPage = candlePage === 1
        return page({ schemaVersion: '1', datasetId: completeDataset.datasetId, candles: [datasetCandles[firstPage ? 0 : 1]], nextCursor: firstPage ? 'page-2' : null, hasMore: firstPage })
      }
    ...
    expect(requested.map((item) => item.path)).toContain('/api/v1/market-data/datasets/10000000-0000-4000-8000-000000000001/candles?pageSize=500&cursor=page-2')
  ...
  it('rejects Candle pages for a different Dataset identity', async () => {
    const api = createBacktestApi({
      fetcher: vi.fn(async () => new Response(JSON.stringify(envelope({
        schemaVersion: '1', datasetId: '20000000-0000-4000-8000-000000000002',
        candles: datasetCandles, nextCursor: null, hasMore: false,
      })), { status: 200 })),
    })

    await expect(api.loadDatasetCandles(completeDataset)).rejects.toThrow(/candles\.datasetId/)
  })

  it('rejects a repeated Dataset Candle continuation cursor', async () => {
    const api = createBacktestApi({
      fetcher: vi.fn(async () => new Response(JSON.stringify(envelope({
        schemaVersion: '1', datasetId: completeDataset.datasetId,
        candles: [datasetCandles[0]], nextCursor: 'same-cursor', hasMore: true,
      })), { status: 200 })),
    })

    await expect(api.loadDatasetCandles(completeDataset)).rejects.toThrow(/repeated continuation cursor/)
  })

  it('rejects an incomplete Candle collection instead of rendering a partial Dataset', async () => {
    const api = createBacktestApi({
      fetcher: vi.fn(async () => new Response(JSON.stringify(envelope({
        schemaVersion: '1', datasetId: completeDataset.datasetId,
        candles: [datasetCandles[0]], nextCursor: null, hasMore: false,
      })), { status: 200 })),
    })

    await expect(api.loadDatasetCandles(completeDataset)).rejects.toThrow(/expected 2 items, received 1/)
  })
})
