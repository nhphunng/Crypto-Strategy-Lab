...
**Purpose**: Make User Stories 5–7 runnable end to end while keeping local secrets in the Git-ignored `.env`, excluding them from source/logs/browser/product payloads, and preventing the generated-strategy sandbox from inheriting application environment or credentials.

- [X] T100 Add fail-closed direct local secret loading from the Git-ignored `.env`, explicit LLM data-policy acknowledgement, persistent encrypted-artifact volume wiring, and configuration/isolation tests for FR-054, ADR-006, the generated-strategy security policy, and Constitution environment/security rules in `backend/src/crypto_lab/infrastructure/settings.py`, `backend/src/crypto_lab/api/dependencies.py`, `backend/src/crypto_lab/infrastructure/sandbox/generated_strategy_runtime.py`, `docker-compose.generated.yml`, `.gitignore`, `.env.example`, and backend contract tests
